...
An earlier, abandoned checking script reported a closed gripper but no object. This illustrates why closure is not equivalent to retention. The final manual script instead lifts to HOVER, stops the chassis, clears previously buffered terminal input and waits up to 30 s for a fresh \code{y}. The decision is based on the current lifted object, not an earlier key press.
