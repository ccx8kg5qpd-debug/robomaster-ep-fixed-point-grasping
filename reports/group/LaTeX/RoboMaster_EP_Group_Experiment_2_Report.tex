...
\dates{Revised: 14 September 2026 \textbullet\ Hardware trials: 12 September 2026}
...
abnormal event in the nominal trials. The video's final empty grasp is a separate,
deliberate object-removal test, as clarified by the team. The nominal 5/5 result
is team-confirmed; no instrumented hardware accuracy, force, payload, or timing data
...
this PDF. Its final segment documents an additional deliberate object-removal
test, excluded from the five nominal transfers. At approximately 58--61 s an
operator removes the target; at approximately 61--69 s the empty gripper closes
and lifts. No subsequent transfer rotation is visible before the clip ends.
The test intent and nominal 5/5 result were clarified by the team. The clip does
not provide a legible terminal transcript or an instrumented stop-time record.
...
motion. Runtime stale-feedback and phase-timeout guards also exist.

\textbf{Hardware case H-01: object removed before grasping.}
The team deliberately removed the target to exercise the grasp-failure path.
The visible empty lift and absence of subsequent transfer are consistent with
holding at the supervised confirmation checkpoint. In the archived SDK script,
only a fresh \codepath{y} authorizes transfer. A negative/blank response or 30 s
timeout aborts above the source without rotation or homing. The \codepath{finally}
block attempts gripper pause, zero wheel speed, and SDK closure. These are
code-defined actions; the exact input, timeout branch, and successful cleanup
calls are not independently established by the video. H-01 is an additional
negative test, not a failed nominal trial or an automatic object detector.

The accompanying abnormal-test record separates observation from expected
behavior. Communication loss, runtime limit violations, stale feedback and
refill failures were not demonstrated as injected system faults. No separate
individual safety-check record was supplied.
...
review, organization, publication, and reporting sequence on 13--14 September 2026;
it is not presented as a fabricated contemporaneous development history.

\begin{figure*}[t]
\centering
\includegraphics[width=0.70\textwidth]{commit_history_excerpt.png}
\caption{Commit-history evidence exported from the repository at revision
\codepath{c232dcd}. The package also includes the text export; a later commit
publishes this revised report.}
\label{fig:commits}
\end{figure*}
...
Abnormal-test record: \codepath{04_results/abnormal_test_record.md} & Included\\
...
and five team-confirmed nominal physical transfers without observed abnormality.
The separate object-removal test demonstrates an empty-grasp scenario with no
subsequent transfer visible; exact terminal and cleanup events remain unlogged. The
...
{Public team repository}, main branch, revision of 14 September 2026.\par
